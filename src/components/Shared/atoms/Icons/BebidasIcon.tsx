import React from 'react';

interface BebidasIconProps {
  className?: string;
  size?: number;
  color?: string;
}

const BebidasIcon: React.FC<BebidasIconProps> = ({
  className = '',
  size = 32,
  color = '#9F9F9F'
}) => {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 62 61"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
      className={className}
    >
      <rect width="62" height="61" fill={color} />
      <defs>
        <pattern id="pattern0_16_1021" patternContentUnits="objectBoundingBox" width="1" height="1">
          <use xlinkHref="#image0_16_1021" transform="matrix(0.01 0 0 0.0101639 0 -0.00819672)" />
        </pattern>
        <image id="image0_16_1021" width="100" height="100" xlinkHref="data:image/png;base64," />
      </defs>
    </svg>
  );
};

export default BebidasIcon;
